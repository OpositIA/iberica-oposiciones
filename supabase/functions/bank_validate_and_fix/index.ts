...
function extractOptionIdFromString(raw: string): QuestionOptionId | null {
  ...
  if (ids.includes(folded as QuestionOptionId))
    return folded as QuestionOptionId;
...
function standardizeOptions(input: unknown): NormalizedOption[] | null {
  ...
  const texts = sliced.map(
    (option) => normalizeOptionText(option) || String(option ?? "").trim()
  );
...
function normalizeCitation(
  ...
  return {
    ...
    reference: String(
      (raw as Record<string, unknown> | null)?.reference ??
        source.reference ??
        ""
    ).trim()
  };
...
function normalizeQuestion(
  ...
  const firstSource = sourceByChunkId.values().next().value as
    | BatchSourceRow
    | undefined;
  ...
  return {
    topicId:
      String(question.topicId ?? question.topic_id ?? fallbackTopicId).trim() ||
      fallbackTopicId,
    topicLabel:
      String(
        question.topicLabel ?? question.topic_label ?? fallbackTopicLabel
      ).trim() || fallbackTopicLabel,
    question: statement,
    options,
...
function buildSourcesBlock(sources: BatchSourceRow[]): string {
  return sources
    .map((source) => {
      const excerpt =
        String(
          (source.raw_chunk as Record<string, unknown> | null)?.content ?? ""
        ).trim() || String(source.snippet ?? "").trim();
      return [
...
function mergeValidationResult(
  ...
  const citations = citationsRaw
    ...
    .filter((citation: Citation | null): citation is Citation =>
      Boolean(citation)
    );

  const explanation =
    String(parsed?.explanation ?? original.explanation).trim() ||
  ...
  return {
    ...
    issues: Array.isArray(parsed?.issues)
      ...
      : [],
    question:
      String(parsed?.question ?? original.question).trim() || original.question,
    options,
    correctOptionId: inferredCorrectOptionId ?? parsedCorrectOptionId,
    explanation,
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: bankRow, error: bankErr } = await supabase
      ...
      .select(
        "id, opposition_id, opposition_name, topic_id, topic_label, difficulty, locale, status, questions, metadata"
      )
      .eq("id", bankTestId)
      .single();
    if (bankErr || !bankRow)
      return json(
        {
          error: `question_bank_tests not found: ${bankErr?.message ?? bankTestId}`
        },
        404
      );
    ...
    const { data: sourceRowsRaw, error: sourceErr } = await supabase
      ...
      .select(
        "id, chunk_id, id_boe, article, reference, unit_type, score, title, snippet, raw_chunk"
      )
      .eq("bank_test_id", bankTestId)
      .order("score", { ascending: false });
    if (sourceErr)
      throw new Error(`Load question_bank_sources error: ${sourceErr.message}`);

    const sourceRows = (
      Array.isArray(sourceRowsRaw) ? sourceRowsRaw : []
    ) as BatchSourceRow[];
    if (!sourceRows.length)
      return json(
        { error: "No question_bank_sources found for this bank_test_id" },
        409
      );

    const sourceByChunkId = new Map(
      sourceRows.map((row) => [row.chunk_id, row] as const)
    );

    const originalQuestions = Array.isArray(bankRow.questions)
      ? [...bankRow.questions]
      : [];
    const inputQuestions = originalQuestions.slice(0, maxQuestions);
    if (!inputQuestions.length)
      return json(
        { error: "No questions found in question_bank_tests.questions" },
        409
      );
    ...
    const normalizedEntries = questionEntries.filter(
      (
        entry
      ): entry is { batchIndex: number; normalized: NormalizedQuestion } =>
        Boolean(entry.normalized)
    );
    if (!normalizedEntries.length)
      return json({ error: "No valid questions to validate" }, 409);
    ...
    for (const { batchIndex, normalized: question } of normalizedEntries) {
      ...
      const candidateSources = citedSources.length
        ? citedSources
        : sourceRows.slice(0, 8);

      const prompt = buildValidationPrompt(
        bankRow as BankTestRow,
        question,
        candidateSources
      );
    ...
    for (const { batchIndex, normalized } of normalizedEntries)
      finalBatchQuestions[batchIndex] =
        correctedByBatchIndex.get(batchIndex) ?? normalized;
    ...
    if (updateBatchErr)
      throw new Error(
        `Update question_bank_tests error: ${updateBatchErr.message}`
      );
    ...
    if (questionRowsErr)
      throw new Error(
        `Load question_bank_questions error: ${questionRowsErr.message}`
      );

    const questionRows = (
      Array.isArray(questionRowsRaw) ? questionRowsRaw : []
    ) as QuestionBankRow[];
    const questionUpdateLimit = Math.min(
      questionRows.length,
      inputQuestions.length
    );
    for (
      let batchIndex = 0;
      batchIndex < questionUpdateLimit;
      batchIndex += 1
    ) {
      ...
      const question =
        correctedByBatchIndex.get(batchIndex) ?? normalizedOriginal;
      ...
      if (updateQuestionErr)
        throw new Error(
          `Update question_bank_questions error: ${updateQuestionErr.message}`
        );
      ...
      if (deleteSourcesErr)
        throw new Error(
          `Delete question_bank_question_sources error: ${deleteSourcesErr.message}`
        );
      ...
      if (sourcePayload.length > 0) {
        ...
        if (insertSourcesErr)
          throw new Error(
            `Insert question_bank_question_sources error: ${insertSourcesErr.message}`
          );
      }
    }
