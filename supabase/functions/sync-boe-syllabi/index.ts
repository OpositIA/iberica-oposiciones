...
function ensureCronSecret(req: Request): Response | null {
  ...
  if (!expected) return json({ error: "Missing CRON_SECRET environment variable" }, 500);
...
function matchesWatchlist(title: string, searchTerms: string[], excludeTerms: string[]): boolean {
  const normalizedTitle = normalizeSearchText(title);
  const normalizedSearchTerms = searchTerms.map(normalizeSearchText).filter(Boolean);
  const normalizedExcludeTerms = excludeTerms.map(normalizeSearchText).filter(Boolean);

  const hasIncludeMatch =
    normalizedSearchTerms.length === 0 ||
    normalizedSearchTerms.some((term) => normalizedTitle.includes(term));

  const hasExcludeMatch = normalizedExcludeTerms.some((term) => normalizedTitle.includes(term));
  return hasIncludeMatch && !hasExcludeMatch;
}

function collectSummaryItems(value: unknown, items: SummaryCandidate[], publishedAt: string): void {
  ...
  const boeId = typeof record.identificador === "string" ? record.identificador : null;
...
async function fetchDailySummaryCandidates(dateKey: string): Promise<SummaryCandidate[]> {
  const response = await fetch(`https://www.boe.es/datosabiertos/api/boe/sumario/${dateKey}`, {
    headers: {
  ...
  if (!response.ok) 
    throw new Error(`BOE summary fetch failed (${response.status}) for ${dateKey}`);
...
function dedupeCandidates(candidates: SummaryCandidate[]): SummaryCandidate[] {
  ...
  for (const candidate of candidates) {
    ...
    const existingScore = Number(existing.publishedAt?.replaceAll("-", "") ?? "0");
    const currentScore = Number(candidate.publishedAt?.replaceAll("-", "") ?? "0");
...
async function getWatchlists(supabase: ReturnType<typeof createClient>, ids?: number[]) {
  let query = supabase
    .from("opposition_watchlist")
    .select("id, opposition_id, label, search_terms, exclude_terms, direct_boe_id, direct_xml_url, search_days_back, is_active")
    .eq("is_active", true)
...
async function discoverCandidatesForWatchlist(watchlist: WatchlistRow): Promise<SummaryCandidate[]> {
  const candidates: SummaryCandidate[] = [];

  if (watchlist.direct_boe_id || watchlist.direct_xml_url) {
    const boeId = watchlist.direct_boe_id ?? deriveBoeIdFromUrl(watchlist.direct_xml_url);
    if (boeId) {
      candidates.push({
        boeId,
        title: watchlist.label,
        xmlUrl: watchlist.direct_xml_url,
        htmlUrl: boeId ? `https://www.boe.es/diario_boe/txt.php?id=${encodeURIComponent(boeId)}` : null,
        publishedAt: null,
        matchedBy: "direct",
      });
    }
  ...
  for (let offset = 0; offset < daysBack; offset += 1) {
    ...
    for (const candidate of dayCandidates) {
      if (!matchesWatchlist(candidate.title, searchTerms, excludeTerms)) continue;
...
function comparePublishedAt(candidatePublishedAt: string | null, currentPublishedAt: string | null): number {
...
function shouldPromoteCandidate(
  ...
  const publishedAtComparison = comparePublishedAt(parsedPublishedAt, currentSyllabus.published_at);
...
async function setCurrentSyllabus(
  ...
  const rpc = await supabase.rpc("set_current_opposition_syllabus", { p_syllabus_id: syllabusId });
  if (!rpc.error) return;
...
async function processCandidate(
  ...
  const shouldBeCurrent = shouldPromoteCandidate(candidate, parsedPublishedAt, currentSyllabus);
  ...
  const existing = await findExistingSyllabus(supabase, watchlist.opposition_id, parsed.sourceHash);
  if (existing) {
  ...
  try {
    const { data: syllabusData, error: syllabusError } = await supabase
      ...
      .insert({
        opposition_id: watchlist.opposition_id,
        boe_id: candidate.boeId,
        source_url: candidate.xmlUrl ?? `https://www.boe.es/diario_boe/xml.php?id=${encodeURIComponent(candidate.boeId)}`,
        published_at: parsedPublishedAt,
    ...
    if (topicIdByCode.size !== topicRows.length) 
      throw new Error("No se pudieron mapear todos los opposition_topics insertados.");
    ...
    const { error: subtopicsError } = await supabase.from("opposition_subtopics").insert(subtopicRows);
    ...
    if (syllabusId !== null) {
      const { error: cleanupError } = await supabase.from("opposition_syllabi").delete().eq("id", syllabusId);
      if (cleanupError) {
...
serve(async (req) => {
  ...
  try {
    ...
    for (const watchlist of watchlists) {
      ...
      let currentSyllabus = await getCurrentSyllabus(supabase, watchlist.opposition_id);
      ...
      for (const candidate of candidates) {
        try {
          ...
          if (result.current_applied === true && typeof result.syllabus_id === "number") {
            currentSyllabus = {
    ...
    const message = error instanceof Error ? error.message : String(error);
    console.error(JSON.stringify({ msg: "sync_boe_syllabi_failed", error: message }));
