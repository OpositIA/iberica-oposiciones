...
type Citation = { chunkId: string; idBoe: string; article: string; reference: string };
...
function requireEdgeSecretIfConfigured(req: Request): Response | null {
  ...
  return got === secret ? null : json({ error: "Unauthorized (x-edge-secret)" }, 401);
}
...
async function ollamaChat(baseUrl: string, model: string, prompt: string) {
  const response = await fetch(baseUrl.replace(/\/$/, "") + "/api/chat", {
    ...
    body: JSON.stringify({
      ...
      format: "json",
      messages: [
        { role: "system", content: "Eres un generador de preguntas de oposiciones. Devuelves SOLO JSON valido." },
        { role: "user", content: prompt },
      ],
      options: { temperature: 0.2, top_p: 0.9, repeat_penalty: 1.1 },
    }),
  });
  if (!response.ok) throw new Error(`Ollama chat error (${response.status}): ${await response.text()}`);
  const data = await response.json();
  if (typeof data?.message?.content !== "string") throw new Error("No message.content in Ollama response");
...
async function embedQuery(text: string) {
  ...
  if (provider === "gemini") {
    ...
    if (!response.ok) throw new Error(`Gemini embed error (${response.status}): ${await response.text()}`);
    const data = await response.json();
    const vector = data?.embedding?.values ?? data?.embedding?.value ?? data?.embedding;
  ...
  if (!response.ok) throw new Error(`Ollama embeddings error (${response.status}): ${await response.text()}`);
  const data = await response.json();
  if (!Array.isArray(data?.embedding)) throw new Error("No embedding in Ollama response");
  return data.embedding as number[];
}

async function loadStructure(supabase: ReturnType<typeof createClient>, oppositionId: string) {
  const { data: syllabus, error: syllabusErr } = await supabase
    ...
    .eq("is_current", true)
    .maybeSingle();
  if (syllabusErr) throw new Error(`Load current syllabus error: ${syllabusErr.message}`);
  ...
  if (topicsErr) throw new Error(`Load syllabus topics error: ${topicsErr.message}`);
  ...
  const { data: subtopicsRaw, error: subtopicsErr } = topics.length
    ? await supabase
        ...
        .select("id, opposition_topic_id, subtopic_code, subtopic_title, section_title")
        .in("opposition_topic_id", topics.map((row) => row.id))
        .order("order_index", { ascending: true })
        .order("id", { ascending: true })
    : { data: [], error: null };
  if (subtopicsErr) throw new Error(`Load syllabus subtopics error: ${subtopicsErr.message}`);
  return { topics, subtopics: (Array.isArray(subtopicsRaw) ? subtopicsRaw : []) as SubtopicRow[] };
}

function blockLabel(topic: TopicRow, byTopic: Map<number, SubtopicRow[]>) {
  const base = s(topic.topic_title) || s(topic.topic_code) || `Bloque ${topic.id}`;
  const section = s(byTopic.get(topic.id)?.find((row) => s(row.section_title))?.section_title);
  return base && section && !key(base).includes(key(section)) ? `${base}. ${section}` : base || section;
}
...
function inferScope(body: RequestBody): Scope {
  ...
  return first(body.block?.id, body.block?.label, body.blockId, body.block_label, body.blockLabel) ? "block" : "topic";
}

function resolveTarget(body: RequestBody, structure: { topics: TopicRow[]; subtopics: SubtopicRow[] } | null): Target {
  ...
  const rawBlockLabel = first(body.block?.label, body.blockLabel, body.block_label);
  const rawTopicId = first(body.topic?.id, body.topicId, body.topic_id);
  const rawTopicLabel = first(body.topic?.label, body.topicLabel, body.topic_label);

  if (!structure) {
    if (scope === "block") {
      const targetId = rawBlockId || rawTopicId;
      const targetLabel = rawBlockLabel || rawTopicLabel;
      if (!targetId || !targetLabel) throw new Error("Falta block.id y block.label");
      return {
        scope, targetId, targetLabel,
        blockId: targetId, blockLabel: targetLabel, topicId: null, topicLabel: null,
        queryText: `${oppositionName} bloque ${targetLabel}`,
        promptInstruction: `La pregunta debe cubrir solo el bloque "${targetLabel}".`,
      };
    }
    if (!rawTopicId || !rawTopicLabel) throw new Error("Falta topic.id y topic.label");
    return {
      scope, targetId: rawTopicId, targetLabel: rawTopicLabel,
      blockId: rawBlockId || null, blockLabel: rawBlockLabel || null, topicId: rawTopicId, topicLabel: rawTopicLabel,
  ...
  for (const row of structure.subtopics) {
    if (!byTopic.has(row.opposition_topic_id)) byTopic.set(row.opposition_topic_id, []);
  ...
  if (scope === "block") {
    ...
    const topic =
      structure.topics.find((row) => wantedId && [s(row.topic_code), s(row.id)].some((value) => key(value) === wantedId)) ??
      structure.topics.find((row) => wantedLabel && [blockLabel(row, byTopic), s(row.topic_title), s(row.topic_code)].some((value) => key(value) === wantedLabel));
    if (!topic) throw new Error(`No se encontro el bloque solicitado en ${body.opposition_id}`);
    const label = blockLabel(topic, byTopic);
    const childTopics = (byTopic.get(topic.id) ?? []).map((row) => s(row.subtopic_title)).filter(Boolean);
    return {
      scope,
  ...
  const subtopic =
    structure.subtopics.find((row) => wantedId && [s(row.subtopic_code), s(row.id)].some((value) => key(value) === wantedId)) ??
    structure.subtopics.find((row) => wantedLabel && key(s(row.subtopic_title)) === wantedLabel);
  if (!subtopic) throw new Error(`No se encontro el tema solicitado en ${body.opposition_id}`);
  const topic = structure.topics.find((row) => row.id === subtopic.opposition_topic_id) ?? null;
...
function prompt(body: RequestBody, target: Target, chunks: Chunk[], recentQuestions: string[]) {
  const used = recentQuestions.length ? recentQuestions.slice(-20).map((q) => `- ${trim(q, 170)}`).join("\n") : "- (ninguna)";
  return `
...
function normalizeOptions(raw: unknown): NormalizedOption[] | null {
  ...
  if (Array.isArray(raw)) {
    ...
    return ids.every((id) => byId.has(id)) ? ids.map((id) => ({ id, text: byId.get(id)! })) : ids.map((id, index) => ({ id, text: built[index].text }));
  }
  return null;
}
...
function normalizeQuestion(raw: unknown, chunksById: Map<string, Chunk>, fallbackChunk: Chunk, target: Target): Question | null {
  ...
  const question = s(record.question ?? record.enunciado ?? record.statement ?? record.prompt ?? record.text);
  ...
  const citations = citationsRaw.map((item) => {
    ...
    const chunk = (chunkId ? chunksById.get(chunkId) ?? [...chunksById.values()].find((row) => row.chunkId.toLowerCase() === chunkId.toLowerCase()) : null) ?? fallbackChunk;
    return {
  ...
  if (!citations.length) 
    citations.push({ chunkId: fallbackChunk.chunkId, idBoe: s(fallbackChunk.idBoe), article: s(fallbackChunk.article), reference: s(fallbackChunk.reference) });
  
  return {
    topicId: s(record.topicId ?? record.topic_id) || target.targetId,
    topicLabel: s(record.topicLabel ?? record.topic_label) || target.targetLabel,
    question,
    options,
    correctOptionId,
    explanation,
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const body: RequestBody = {
      ...
      locale: sanitizeCode(parsedBody.locale ?? "es", 8) === "es" ? "es" : undefined,
      difficulty: sanitizeCode(parsedBody.difficulty, 16) as Difficulty,
      count: sanitizeInteger(parsedBody.count, { min: 1, max: 100, fallback: 1 }) ?? 1,
      scope: parsedBody.scope === "block" || parsedBody.scope === "topic" ? parsedBody.scope : undefined,
      target_type:
      ...
      match_threshold:
        typeof parsedBody.match_threshold === "number" && Number.isFinite(parsedBody.match_threshold)
          ? clamp(parsedBody.match_threshold, 0, 1)
          : undefined,
      match_count:
        sanitizeInteger(parsedBody.match_count, { min: 1, max: 120, fallback: 80 }) ?? 80,
      context_chars:
        sanitizeInteger(parsedBody.context_chars, { min: 2000, max: 30000, fallback: 16000 })
        ?? 16000,
      avoid_recent_bank_tests:
        sanitizeInteger(parsedBody.avoid_recent_bank_tests, { min: 0, max: 30, fallback: 8 })
        ?? 8,
    };
    const locale = body.locale ?? "es";
    if (locale !== "es") return json({ error: "Solo se permite locale=es" }, 400);
    if (!body.opposition_id) return json({ error: "opposition_id required" }, 400);
    if (!["facil", "media", "dificil"].includes(body.difficulty)) return json({ error: "difficulty must be facil|media|dificil" }, 400);
    ...
    const recentQuestions = (Array.isArray(previousRows) ? previousRows : []).flatMap((row: any) =>
      Array.isArray(row?.questions) ? row.questions.map((question: any) => s(question?.question)).filter(Boolean) : []
    );

    const embedding = await embedQuery(target.queryText);
    const { data: ragRows, error: ragErr } = await supabase.rpc("buscar_articulos", {
      query_embedding: embedding,
      match_threshold: body.match_threshold ?? 0.25,
      match_count: body.match_count ?? 80,
    });
    if (ragErr) throw new Error(`RPC buscar_articulos error: ${ragErr.message}`);

    const chunks = (Array.isArray(ragRows) ? ragRows : []).map((row: any) => {
      const content = s(row.content ?? row.contenido ?? row.text ?? row.raw_text ?? row.snippet);
      if (!content) return null;
      return {
        chunkId: s(row.chunkId ?? row.chunk_id ?? row.id) || crypto.randomUUID(),
        idBoe: s(row.idBoe ?? row.id_boe ?? row.boe_id) || "unknown",
        article: s(row.article ?? row.articulo ?? row.articulo_num) || null,
        reference: s(row.reference ?? row.referencia ?? row.apartado_path) || null,
        unitType: s(row.unit_type ?? row.unitType) || null,
        score: typeof row.score === "number" ? row.score : (typeof row.similarity === "number" ? row.similarity : null),
        title: s(row.title ?? row.titulo ?? row.titulo_ley) || null,
        snippet: s(row.snippet) || null,
        content,
        rawChunk: row,
      } as Chunk;
    }).filter((row): row is Chunk => Boolean(row)).sort((a, b) => (b.score ?? 0) - (a.score ?? 0)).slice(0, 40);

    if (!chunks.length) return json({ error: "No RAG chunks returned" }, 500);

    const llmText = await ollamaChat(ollamaBaseUrl, ollamaModel, prompt(body, target, chunks, recentQuestions));
    const parsed = safeJsonParse(llmText);
    const generatedRaw = Array.isArray(parsed?.questions) ? parsed.questions : [];
    ...
    const questions = generatedRaw
      .map((question) => normalizeQuestion(question, chunksById, fallbackChunk, target))
      .filter((question): question is Question => Boolean(question))
      .slice(0, 1);
    if (!questions.length) return json({ error: "Could not generate any valid questions" }, 500);
    ...
    const { data: testRow, error: testErr } = await supabase
      ...
      .insert({
        ...
        metadata: {
          ...
          rag: { match_threshold: body.match_threshold ?? 0.25, match_count: body.match_count ?? 80 },
          generator: { mode: "single-question", requestedCount, generatedCount: questions.length },
          generated_at: nowIso,
        },
        questions,
      })
      .select("id")
      .single();
    if (testErr) throw new Error(`Insert question_bank_tests error: ${testErr.message}`);
    const bankTestId = (testRow as any)?.id;

    const { error: sourcesErr } = await supabase.from("question_bank_sources").insert(
      chunks.map((chunk) => ({
    ...
    if (sourcesErr) throw new Error(`Insert question_bank_sources error: ${sourcesErr.message}`);
    ...
    const { data: insertedQuestionsRaw, error: questionErr } = await supabase
      ...
      .select("id, citations");
    if (questionErr) throw new Error(`Insert question_bank_questions error: ${questionErr.message}`);

    const questionSources = (Array.isArray(insertedQuestionsRaw) ? insertedQuestionsRaw : []).flatMap((row: any) => {
      const questionId = Number(row?.id);
      const citations = Array.isArray(row?.citations) ? row.citations : [];
      return citations.map((citation: any) => {
        const chunk = chunksById.get(s(citation?.chunkId ?? citation?.chunk_id));
        return {
          question_id: questionId,
          chunk_id: s(citation?.chunkId ?? citation?.chunk_id),
          id_boe: s(citation?.idBoe ?? citation?.id_boe ?? chunk?.idBoe) || null,
    ...
    if (questionSources.length) {
      const { error: questionSourcesErr } = await supabase.from("question_bank_question_sources").insert(questionSources);
      if (questionSourcesErr) throw new Error(`Insert question_bank_question_sources error: ${questionSourcesErr.message}`);
    }
    ...
    return json({
      ...
      question_pool_inserted: Array.isArray(insertedQuestionsRaw) ? insertedQuestionsRaw.length : 0,
      status: "draft",
      target: {
        scope: target.scope,
        id: target.targetId,
