...
with params as (
  select
    ...
    greatest(96, least(greatest(1, least(coalesce(match_count, 18), 200)) * 24, 4000))::integer as candidate_k,
...
candidate_chunks as (
  ...
  where rc.is_current = true
    ...
    and (
      p.unit_type_filter = ''
      or lower(regexp_replace(coalesce(rc.metadata->>'unit_type', 'estructura'), '[^a-z0-9]+', '', 'g')) = p.unit_type_filter
    )
...
with params as (
  select
    greatest(1, least(coalesce(match_count, 18), 200))::integer as k,
    greatest(0.0, least(coalesce(match_threshold, 0.0), 1.0))::double precision as threshold,
    greatest(120, least(greatest(1, least(coalesce(match_count, 18), 200)) * 28, 4000))::integer as candidate_k,
    greatest(24, least(greatest(1, least(coalesce(match_count, 18), 200)) * 8, 800))::integer as structured_k,
    subvector(query_embedding, 1, 2000)::vector(2000) as query_subvector,
    upper(nullif(trim(filter_id_boe), '')) as boe_filter,
    lower(
      regexp_replace(
...
annotated as (
  select
    ...
    lower(regexp_replace(replace(coalesce(cc.metadata->>'article', cc.title, ''), ',', '.'), '[^0-9a-z.]+', '', 'g')) as articulo_num_compact,
    lower(regexp_replace(coalesce(cc.metadata->>'unit_id', ''), '[^0-9a-z]+', '', 'g')) as unit_id_compact,
    lower(regexp_replace(coalesce(cc.metadata->>'apartado_path', ''), '[^0-9a-z]+', '', 'g')) as apartado_path_compact,
    coalesce(
      nullif(cc.metadata->>'unit_type', ''),
      'estructura'
    ) as unit_type,
